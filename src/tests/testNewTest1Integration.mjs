...
section('Integrità Globale Banca 392 (con 65 da NEW Test1.pdf)');
check(questionBank.length === 392, `Totale quesiti esatto: 392 (trovati: ${questionBank.length})`);
...
check(invalidSubjects.length === 0, 'Tutti i 392 quesiti appartengono esclusivamente alle 4 materie ammesse');

const subjectCounts = {};
questionBank.forEach(q => subjectCounts[q.materia] = (subjectCounts[q.materia] || 0) + 1);
check(subjectCounts['Matematica'] === 193, `Matematica fisica: 193 (118 base + 25 M1 + 25 M2 + 25 M3) (trovati: ${subjectCounts['Matematica']})`);
